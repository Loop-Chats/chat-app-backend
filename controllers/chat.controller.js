  ...
  try {
    ...
    const updatedUserChats = userChats.map((chat) => {
      if (!chat.isGroupChat) {
        ...
        return {
          ...chat,
          chatName: otherUser ? otherUser.username : "Unknown Operator",
    ...
    res.status(200).json(updatedUserChats);
  } catch (error) {
...
export const createUserChat = async (req, res) => {
  try {
    ...
    if (
      ...
      return res
        .status(400)
        .json({ message: "At least one other user ID is required" });
    }
    ...
    res.status(201).json(chatDataToReturn);
  } catch (error) {
...
export const deleteUserChat = async (req, res) => {
  try {
    ...
    if (!chat.isGroupChat) {
      return res
        .status(400)
        .json({ message: "Cannot delete a non-group chat" });
    }

    if (
      !chat.users.includes(userId) ||
      userId.toString() !== chat.groupAdmin.toString()
    ) {
      return res
        .status(403)
